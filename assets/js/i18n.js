...
(function () {
  ...
  // Namespaces whose copy is generated by their own page script, which listens
  // for the <html lang> change instead of consuming a JSON bundle.
  var SELF_RENDERED = { pricing: 1, download: 1 };
  ...
  function load(code) {
    ...
    if (SELF_RENDERED[NS]) {
      // No bundle to fetch: just record the choice and flip lang/dir so the
      // page script re-renders itself from its own dictionary.
      current = code;
      localStorage.setItem(STORE_KEY, code);
      setDir();
      updateSwitcher();
  ...
  var AVAILABLE = {
    ...
    privacy: ['en','zh'],
    refund: ['en','zh'],
    // pricing/download build their own copy in pricing.js / download.js and
    // re-render when <html lang> flips, so they need no JSON bundle.
    pricing: ['en','zh'],
    download: ['en','zh']
  };
